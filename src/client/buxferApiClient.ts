...
import {
  ...
  preprocessBuxferTransactions as processBuxferTransactions,
...
export class BuxferApiClient {
  ...
  private isTokenExpired(): boolean {
    ...
    const timeDifferenceInMilliseconds = Math.abs(
      nowDate.getTime() - this.authTokenCreation.getTime(),
    );
  ...
  private async refreshToken(endpoint: string) {
    if (
      endpoint != "login" &&
      (this.authToken == null || this.isTokenExpired())
    ) {
  ...
  private async makeApiRequest<T>(
    ...
    try {
      let response = (await axios.request<BuxferResponseContainer>(config))
        .data;
      return response.response;
    } catch (error) {
      if (error instanceof Error) {
        this.log(
          `Error making ${method} API request: ${error.message}`,
          "error",
        );
      } else {
        this.log("Unknown error occurred", "error");
  ...
  /**
   * Send deduplicated and batched transactions in multiple POST requests in parallel
  ...
  private async deduplicateAddTransactionBulks(
    ...
    const params: GetTransactionsQueryParameters =
      new GetTransactionsQueryParameters();
    params.startDate = startDate;
    params.endDate = endDate;
    const dbTransactions: BuxferTransaction[] =
      await this.getTransactions(params);

    // Deduplicate transactions
    const processedTransactions: ProcessedTransactions =
      processBuxferTransactions(scrappedTransactions, dbTransactions, true);
    const response = await this.addTransactionBulks(
      processedTransactions.deduplicatedTransactions,
    );
    response.duplicatedTransactionIds =
      processedTransactions.duplicatedTransactions.map((trx) =>
        trx.id ? trx.id.toString() : "",
      );
    response.ignoredTransactionIds =
      processedTransactions.futureTransactions.map((trx) =>
        trx.id ? trx.id.toString() : "",
      );
    return response;
  }

  private async addTransactionBulks(
    ...
    for (let i = 0; i < bodies.length; i += this.batchSize) {
      ...
      const promises = batch.map((body) =>
        this.makeApiRequest<BuxferTransaction>("transaction_add", "POST", body),
      );
      ...
      await Promise.all(promises)
        .then((responses) => {
          this.log(
            `Batch ${batchIndex} completed: ${responses.length} transactions updated`,
            "info",
          );
          responseContainer.successfulBatches++;
          responses.forEach((trx) => {
            if (trx.id) {
              // Returned transactions from Buxfer should include ID's
  ...
  async deleteTransaction(transactionId: string): Promise<Object> {
    return await this.makeApiRequest<Object>(
      "transaction_delete",
      "POST",
      { id: transactionId },
      undefined,
  ...
  ): Promise<BuxferTransaction[]> {
    let response = await this.makeApiRequest<GetTransactionsResponseData>(
      "transactions",
      "GET",
      undefined,
      queryParams,
  ...
  async getAccounts(): Promise<BuxferAccount[]> {
    let response = await this.makeApiRequest<GetAccountsResponseData>(
      "accounts",
      "GET",
    );
  ...
  async getBudgets(): Promise<BuxferBudget[]> {
    let response = await this.makeApiRequest<GetBudgetsResponseData>(
      "budgets",
      "GET",
    );
  ...
  async getTags(): Promise<BuxferTag[]> {
    let response = await this.makeApiRequest<GetTagsResponseData>(
      "tags",
      "GET",
    );
  ...
  async getLoans(): Promise<BuxferLoan[]> {
    let response = await this.makeApiRequest<GetLoansResponseData>(
      "loans",
      "GET",
    );
  ...
  async getReminders(): Promise<BuxferReminder[]> {
    let response = await this.makeApiRequest<GetRemindersResponseData>(
      "reminders",
      "GET",
    );
  ...
  async getContacts(): Promise<BuxferContact[]> {
    let response = await this.makeApiRequest<GetContactsResponseData>(
      "contacts",
      "GET",
    );
  ...
  async getGroups(): Promise<BuxferGroup[]> {
    let response = await this.makeApiRequest<GetGroupsResponseData>(
      "groups",
      "GET",
    );
